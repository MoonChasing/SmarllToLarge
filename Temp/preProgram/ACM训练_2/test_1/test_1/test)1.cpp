...
bool ifhave(int nx , int ny)
{
	if(maze[nx][ny] >='A' && maze[nx][ny] <='J' && if_exist(maze[nx][ny]+32) )
		return true;
	else
		return false;
}
int bfs()
{
	...
	while(que.size())
	{
		...
		for(int i=0; i<4; i++)
		{
			...
			{
				if(maze[nx][ny] == '.' ||(maze[nx][ny]>='a'	 && maze[nx][ny] <= 'j') || maze[nx][ny] == '@' || maze[nx][ny] == '^' || ifhave(nx,ny) )
				{
